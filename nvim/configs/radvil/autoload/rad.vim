...
function! rad#easy_autoload() abort
...
""
" List occurences
function! rad#list_occurrences(...) abort
  if a:0 > 0
...
""
" Find project root
function! rad#find_project_root(...) abort
...
function! rad#dict_to_formatted_json(dict) abort
  ...
  call nvim_buf_set_lines(l:buffer_number, 0, -1, 1, std#json#format(rad#json_encode(a:dict)))

  silent! call dictwatcherdel(a:dict, '*', 's:dict_watcher_func')

  function! s:dict_watcher_func(d, k, z) abort closure
    call nvim_buf_set_lines(l:buffer_number, 1, -1, 1, std#json#format(rad#json_encode(a:d)))
  endfunction
...
function! rad#profile( command, numberOfTimes )
...
function! rad#code_review_yank() abort
...
function! rad#visual_code_review() abort
...
vnoremap yc <c-r>:call rad#visual_code_review()<CR>
xnoremap yc <c-r>:call rad#visual_code_review()<CR>

if !exists('*rad#save_and_exec')
  function! rad#save_and_exec() abort
    if &filetype == 'vim'
      :silent! write
      :source %
    elseif &filetype == 'lua'
...
function! rad#jump_direction(letter)
